import React from 'react'

export default function FooterMobile() {
  return (
    
      <div className="">
      <footer class="bg-white bg-footer">
        
        <div  className="MuiGrid-root MuiGrid-container text-white text-xs flex ml-60 border-t-2 pt-5 ">
          <div className="MuiGrid-root jss147 MuiGrid-item MuiGrid-grid-xs-12 MuiGrid-grid-sm-3 MuiGrid-grid-lg-2 mr-16">
            <img
              alt
              src="data:image/jpeg;base64,"
              width={100}
            />
          </div>
          <div className="MuiGrid-root MuiGrid-item MuiGrid-grid-xs-12 MuiGrid-grid-sm-6 MuiGrid-grid-lg-8 mr-16 ">
            <h6 className="MuiTypography-root jss143 MuiTypography-h6">
              TIX – SẢN PHẨM CỦA CÔNG TY CỔ PHẦN ZION
            </h6>
            <h6 className="MuiTypography-root MuiTypography-h6">
              Địa chỉ: Z06 Đường số 13, Phường Tân Thuận Đông, Quận 7, Tp. Hồ
              Chí Minh, Việt Nam.
            </h6>
            <h6 className="MuiTypography-root MuiTypography-h6">
              Giấy chứng nhận đăng ký kinh doanh số: 0101659783,
            </h6>
            <h6 className="MuiTypography-root MuiTypography-h6">
              đăng ký thay đổi lần thứ 30, ngày 22 tháng 01 năm 2020 do Sở kế
              hoạch và đầu tư Thành phố Hồ Chí Minh cấp.
            </h6>
            <h6 className="MuiTypography-root MuiTypography-h6">
              Số Điện Thoại (Hotline): 1900 545 436
            </h6>
          </div>
          <div className="MuiGrid-root jss147 MuiGrid-item MuiGrid-grid-xs-12 MuiGrid-grid-sm-3 MuiGrid-grid-lg-2 mr-16">
            <img
              alt
              src="https://demo1.cybersoft.edu.vn/static/media/daThongBao-logo.cb85045e.png"
              width={100}
            />
          </div>
        </div>
      </footer>
    </div>
    
  )
}
